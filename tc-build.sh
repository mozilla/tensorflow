...
pushd ${DS_ROOT_TASK}/DeepSpeech/tf/
    ...
    if [ "${build_ios_arm64}" = "yes" ]; then
        echo "" | TF_NEED_CUDA=0 ./configure && ${BAZEL_BUILD} -c opt ${BAZEL_IOS_ARM64_FLAGS} ${BAZEL_EXTRA_FLAGS} ${BUILD_TARGET_LITE_LIB}
    fi;

    if [ "${build_ios_x86_64}" = "yes" ]; then
        echo "" | TF_NEED_CUDA=0 ./configure && ${BAZEL_BUILD} -c opt ${BAZEL_IOS_X86_64_FLAGS} ${BAZEL_EXTRA_FLAGS} ${BUILD_TARGET_LITE_LIB}
    fi;
